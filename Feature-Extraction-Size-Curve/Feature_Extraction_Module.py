...
label_set_new = np.concatenate([label_set_0,label_set_1[0:7000]], axis=0)
#label_set_new = label_set_1
...
X_train, X_test = train_test_split(label_set_new, test_size=0.4, random_state=rand_int)
X_test,  X_cv   = train_test_split(X_test,        test_size=0.5, random_state=rand_int)

#%% Save the testing set

Test47  = np.load('Test_set_47_2000.npy')
Test64  = np.load('Test_set_64_2000.npy')
Test109 = np.load('Test_set_109_2000.npy')
...
def define_network(g_model, d_model):
...
whole_model = define_network(model1, model2)
...
whole_model.compile(optimizer=opt, loss='mean_squared_error', metrics=['accuracy'])

epoch   = 350
history = whole_model.fit(X_train, X_train, batch_size=64, epochs=epoch, 
                    steps_per_epoch=40, validation_data=(X_cv, X_cv))

#%% Generate the prediction from specified test set

predict = whole_model.predict(X_test_set)

score = whole_model.evaluate(X_test_set, X_test_set, verbose=1)
print('\n', 'Test accuracy', score)
...
#model1.save('conv_model_128_030402023_rn64.h5')
#model2.save('deconv_model_128_03042023_rn64.h5')

#%% Evaluate the prediction accuracy

[m,n,p] = X_test_set.shape

pred_image = np.round(predict[:,:,:,0])
error = np.zeros((m,1))
...
for i in range(m):
    ...
    for j in range(n):
        for k in range(p):
            ...
            if X_test_set[i,j,k] == pred_image[i,j,k]:
                num = num+1
                
    error[i] = (n*p - num) / (n*p)
                
mean_error = np.mean(error)
print('The mean error rate is', mean_error)
    
#%% Evaludate the prediction error of different regions
...
#             if X_test[i,j,k] == 0:
#                 num0_true = num0_true + 1
#                 if X_test[i,j,k] == pred_image[i,j,k]:
#                     num0 = num0 + 1
                    
#             elif X_test[i,j,k] == 1:
#                 num1_true = num1_true + 1
#                 if X_test[i,j,k] == pred_image[i,j,k]:
#                     num1 = num1 + 1
                   
#             elif X_test[i,j,k] == 2:
#                 num2_true = num2_true + 1
#                 if X_test[i,j,k] == pred_image[i,j,k]:
#                     num2 = num2 + 1
     
#     error0[i] = 1 - num0 / num0_true 
#     error1[i] = 1 - num1 / num1_true
#     error2[i] = 1 - num2 / num2_true       
    
# mean_error0 = np.mean(error0)
# mean_error1 = np.mean(error1)
# mean_error2 = np.mean(error2)

# print('The mean error rate of cutout is', mean_error0)
# print('The mean error rate of matrix is', mean_error1)
# print('The mean error rate of ring is', mean_error2)   
